...
const StatCard = ({ label, value, sub, color = 'blue', link, icon }) => (
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard
          label="My Room"
          value={profile?.roomAllocation?.room?.roomNumber || 'Not Assigned'}
        ...
        <StatCard
          ...
          sub="Tap to pay now"
          color={pendingPayments > 0 ? 'red' : 'green'}
        ...
        <StatCard
          ...
          sub={openComplaints > 0 ? 'Being processed' : 'All resolved'}
          color={openComplaints > 0 ? 'yellow' : 'green'}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {[
          { to: '/student/out-pass', icon: '🚪', label: 'Apply Out Pass' },
          { to: '/student/home-pass', icon: '🏡', label: 'Apply Home Pass' },
          { to: '/student/food', icon: '🍽️', label: 'View Food Menu' },
          { to: '/student/laundry', icon: '👕', label: 'Laundry Request' },
          { to: '/student/complaints', icon: '📋', label: 'File Complaint' },
          { to: '/student/payments', icon: '💳', label: 'Make Payment' },
        ].map(({ to, icon, label }) => (
          <Link key={to} to={to}
